...
core.nvim targets *only* the latest
...
> `https://github.com/<your_github_username>/core.nvim.git`

You likely want to remove `lazy-lock.json` from your fork's `.gitignore` file
too - it's ignored in the core repo to make maintenance easier, but it's
...
```sh
git clone https://github.com/<your_github_username>/core.nvim.git "${XDG_CONFIG_HOME:-$HOME/.config}"/nvim
```
...
```
git clone https://github.com/<your_github_username>/core.nvim.git "%localappdata%\nvim"
```

If you're using `powershell.exe`

```
git clone https://github.com/<your_github_username>/core.nvim.git "${env:LOCALAPPDATA}\nvim"
```
...
* Can I keep my existing configuration in parallel to core.nvim?
  * Yes! You can use [NVIM_APPNAME](https://neovim.io/doc/user/starting.html#%24NVIM_APPNAME)`=nvim-NAME`
    to maintain multiple configurations. For example, you can install the core
    configuration in `~/.config/nvim-core` and create an alias:
    ```
    alias nvim-core='NVIM_APPNAME="nvim-core" nvim'
    ```
    When you run Neovim using `nvim-core` it will use the alternative
    config directory and the matching local directory
    `~/.local/share/nvim-core`. You can apply this approach to any Neovim
    distribution that you would like to try out.
* What if I want to "uninstall" this configuration:
  * See [lazy.nvim uninstall](https://lazy.folke.io/usage#-uninstalling) information
* Why is the core.nvim `init.lua` a single file? Wouldn't it make sense to split it into multiple files?
  * The main purpose of core.nvim is to serve as a teaching tool and a reference
    configuration that someone can easily use to `git clone` as a basis for their own.
    As you progress in learning Neovim and Lua, you might consider splitting `init.lua`
    into smaller parts. A modular fork of this style of config that does this while maintaining the
    same functionality is available here:
    * a modular upstream fork of this approach
  * Discussions on this topic can be found here:
    * the upstream restructure discussion
    * the upstream multi-file refactor discussion

### Install Recipes

Below you can find OS specific install instructions for Neovim and dependencies.

After installing all the dependencies continue with the [Install core.nvim](#install-core.nvim) step.
...
in the upstream Neovim community discussions.
